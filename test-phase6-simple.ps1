...
Write-Host "`nPhase 6.3: User System & Community Features" -ForegroundColor Yellow
Write-Host "===============================================" -ForegroundColor Yellow

# Test 1: Authentication System
Write-Host "`nTest 1: Authentication System" -ForegroundColor Cyan
try {
    # Register user
    $registerData = @{
    ...
    $response = Invoke-RestMethod -Uri "$API_BASE/api/auth/signup" -Method POST -Body $registerData -ContentType "application/json"
...
Write-Host "`nTest 2: Session Management" -ForegroundColor Cyan
try {
    $headers = @{
...
Write-Host "`nTest 3: Social Features" -ForegroundColor Cyan
try {
    # Get public sessions
...
Write-Host "`nTest 4: Profile Management" -ForegroundColor Cyan
try {
...
Write-Host "`nPhase 6.4: Security Features" -ForegroundColor Yellow
Write-Host "=============================" -ForegroundColor Yellow

# Test 1: Rate Limiting
Write-Host "`nTest 1: Rate Limiting" -ForegroundColor Cyan
try {
    $rateLimitTest = @()
    for ($i = 0; $i -lt 25; $i++) {
        try {
...
Write-Host "`nTest 2: Input Validation" -ForegroundColor Cyan
try {
    $invalidData = @{
        birth_data = @{
            date = "invalid-date"
...
# Test 3: CORS & Headers
Write-Host "`nTest 3: CORS & Headers" -ForegroundColor Cyan
try {
...
Write-Host "`nMonetization System" -ForegroundColor Yellow
Write-Host "====================" -ForegroundColor Yellow

# Test 1: Subscription Plans
Write-Host "`nTest 1: Subscription Plans" -ForegroundColor Cyan
try {
...
Write-Host "`nTest 2: Feature Limits" -ForegroundColor Cyan
try {
...
Write-Host "`nTest 3: Export Controls" -ForegroundColor Cyan
try {
    $exportData = @{
        format = "midi"
        quality = "high"
...
# Test 4: Payment Flow
Write-Host "`nTest 4: Payment Flow" -ForegroundColor Cyan
try {
    $upgradeData = @{
        planId = "pro"
        paymentMethod = "demo"
...
Write-Host "`nMusic Composition - 30s Per House Rule" -ForegroundColor Yellow
Write-Host "=========================================" -ForegroundColor Yellow

# Test 1: 30s Per House Rule
Write-Host "`nTest 1: 30s Per House Rule" -ForegroundColor Cyan
try {
    $melodicData = @{
        chart_data = $testChart
        configuration = @{
...
Write-Host "`nTest 2: Melodic Generation" -ForegroundColor Cyan
try {
    $response = Invoke-RestMethod -Uri "$API_BASE/api/audio/melodic" -Method POST -Body $melodicData -ContentType "application/json"
...
Write-Host "`nTest 3: Audio Controls" -ForegroundColor Cyan
try {
    # Check audio status
...
Write-Host "`nTest 4: Export Formats" -ForegroundColor Cyan
try {
...
Write-Host "`nBlind Spot Detection" -ForegroundColor Yellow
Write-Host "=====================" -ForegroundColor Yellow

# Test 1: Session Sharing Logic
Write-Host "`nTest 1: Session Sharing Logic" -ForegroundColor Cyan
try {
...
Write-Host "`nTest 2: Rate Limit Bypass" -ForegroundColor Cyan
try {
...
Write-Host "`nTest 3: Export Filename Conflicts" -ForegroundColor Cyan
try {
    $exportData1 = @{
        format = "wav"
        quality = "high"
...
Write-Host "`nTest 4: Cross-User Access" -ForegroundColor Cyan
try {
...
Write-Host "`nTest 5: Mobile Compatibility" -ForegroundColor Cyan
try {
...
Write-Host "`nPhase 6.3-6.4 Final Validation Complete!" -ForegroundColor Cyan
Write-Host "=============================================" -ForegroundColor Cyan
...
Write-Host "`nOverall Success Rate: $([math]::Round($overallSuccess, 1))%" -ForegroundColor $(if ($overallSuccess -ge 90) { "Green" } else { "Yellow" })

# Phase 7 Readiness Assessment
Write-Host "`nPhase 7 Readiness Assessment:" -ForegroundColor Cyan
...
if ($overallSuccess -ge 95) {
    ...
    Write-Host "   Real-Time Composition: CLEARED" -ForegroundColor Green
    Write-Host "   Cross-Platform Playback: CLEARED" -ForegroundColor Green
} elseif ($overallSuccess -ge 85) {
    Write-Host "   GOOD - Minor issues to address before Phase 7" -ForegroundColor Yellow
...
Write-Host "`nPhase 6.3-6.4 Implementation Status:" -ForegroundColor Cyan
...
Write-Host "`nMusic Composition Defaults:" -ForegroundColor Cyan
...
Write-Host "`nSecurity & Monetization:" -ForegroundColor Cyan
...
Write-Host "`nReady for Phase 7: Mobile Support + Real-Time Jam Sessions!" -ForegroundColor Green 