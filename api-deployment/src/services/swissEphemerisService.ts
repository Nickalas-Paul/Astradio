...
class SwissEphemerisService {
  ...
  // Public method to generate daily chart from ephemeris
  async getDailyChartFromEphemeris(date: string): Promise<AstroChart> {
    const [year, month, day] = date.split('-').map(Number);
    const chartDate = new Date(year, month - 1, day, 12, 0); // Noon on the specified date
    ...
    const planets = await this.calculatePlanetaryPositions(chartDate, latitude, longitude, timezone);
    const houses = await this.calculateHouseCusps(chartDate, latitude, longitude, timezone);
    const aspects = this.calculateAspects(planets);
    
    return this.convertToAstroChart(planets, houses, aspects, {
      date,
      time: '12:00',
      latitude,
      longitude,
