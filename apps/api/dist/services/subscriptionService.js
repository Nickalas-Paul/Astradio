...
class SubscriptionService {
     ...
     */
    static async createCheckoutSession(userId, planId) {
        const db = await (0, database_1.getDatabase)();
        ...
        await db.run('INSERT INTO checkout_sessions (id, user_id, plan_id, amount, status, created_at) VALUES (?, ?, ?, ?, ?, ?)', [checkoutSession.id, checkoutSession.userId, checkoutSession.planId, checkoutSession.amount, checkoutSession.status, checkoutSession.createdAt.toISOString()]);
        return checkoutSession;
    }
    /**
     * Complete checkout session (called by Stripe webhook)
     */
    static async completeCheckoutSession(sessionId, stripeSessionId) {
        const db = await (0, database_1.getDatabase)();
        ...
        await db.run('UPDATE checkout_sessions SET status = ?, stripe_session_id = ? WHERE id = ?', ['completed', stripeSessionId, sessionId]);
        ...
        await db.run('UPDATE users SET subscription_plan = ?, subscription_status = ?, subscription_expires_at = ?, updated_at = ? WHERE id = ?', [plan.id, 'active', endDate.toISOString(), new Date().toISOString(), session.user_id]);
        // Log activity
    ...
    static async cancelSubscription(userId) {
        const db = await (0, database_1.getDatabase)();
        await db.run('UPDATE users SET subscription_status = ?, updated_at = ? WHERE id = ?', ['cancelled', new Date().toISOString(), userId]);
    ...
    static async downgradeToFree(userId) {
        const db = await (0, database_1.getDatabase)();
        await db.run('UPDATE users SET subscription_plan = ?, subscription_status = ?, subscription_expires_at = NULL, updated_at = ? WHERE id = ?', ['free', 'active', new Date().toISOString(), userId]);
        await this.logSubscriptionActivity(userId, 'subscription_downgraded', { planId: 'free' });
    }
    /**
     * Track usage for a user
     */
    static async trackUsage(userId, action) {
        const db = await (0, database_1.getDatabase)();
        // Log the activity
        await db.run('INSERT INTO user_activity (id, user_id, action, details, ip_address, user_agent, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)', [(0, uuid_1.v4)(), userId, action, JSON.stringify({}), '127.0.0.1', 'AstroAudio-API', new Date().toISOString()]);
    ...
    static async getUserUsage(userId) {
        ...
        const chartGenerations = await db.get('SELECT COUNT(*) as count FROM user_activity WHERE user_id = ? AND action = ?', [userId, 'chart_generated']);
        const audioCreated = await db.get('SELECT COUNT(*) as count FROM user_activity WHERE user_id = ? AND action = ?', [userId, 'audio_created']);
        const exports = await db.get('SELECT COUNT(*) as count FROM user_exports WHERE user_id = ?', [userId]);
        const sandboxUses = await db.get('SELECT COUNT(*) as count FROM user_activity WHERE user_id = ? AND action = ?', [userId, 'sandbox_used']);
        return {
     ...
     */
    static async logSubscriptionActivity(userId, action, details) {
        try {
            const db = await (0, database_1.getDatabase)();
