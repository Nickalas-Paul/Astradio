...
class SessionService {
    ...
    static async getUserSessions(userId, limit = 20, offset = 0) {
        const db = await (0, database_1.getDatabase)();
        const sessions = await db.all('SELECT * FROM sessions WHERE user_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?', [userId, limit, offset]);
     ...
     * Get public sessions
     */
    static async getPublicSessions(limit = 20, offset = 0, sortBy = 'created_at') {
        const db = await (0, database_1.getDatabase)();
        const sessions = await db.all(`SELECT * FROM sessions WHERE is_public = 1 ORDER BY ${sortBy} DESC LIMIT ? OFFSET ?`, [limit, offset]);
        return sessions.map((session) => ({
            ...session,
            chart_data: JSON.parse(session.chart_data),
            audio_config: session.audio_config ? JSON.parse(session.audio_config) : undefined,
    ...
    static async updateSession(sessionId, userId, updates) {
        ...
        await db.run(`UPDATE sessions SET ${updateFields}, updated_at = ? WHERE id = ? AND user_id = ?`, [...processedValues, new Date().toISOString(), sessionId, userId]);
     ...
     */
    static async incrementPlayCount(sessionId) {
        const db = await (0, database_1.getDatabase)();
     ...
     * Like/unlike session
     */
    static async toggleLike(sessionId, userId) {
        const db = await (0, database_1.getDatabase)();
        // Check if already liked
        const existingLike = await db.get('SELECT * FROM session_likes WHERE session_id = ? AND user_id = ?', [sessionId, userId]);
        if (existingLike) {
            // Unlike
            await db.run('DELETE FROM session_likes WHERE session_id = ? AND user_id = ?', [sessionId, userId]);
            ...
            // Like
            await db.run('INSERT INTO session_likes (session_id, user_id) VALUES (?, ?)', [sessionId, userId]);
            await db.run('UPDATE sessions SET like_count = like_count + 1 WHERE id = ?', [sessionId]);
            const session = await db.get('SELECT like_count FROM sessions WHERE id = ?', [sessionId]);
     ...
     */
    static async addComment(sessionId, userId, comment) {
        const db = await (0, database_1.getDatabase)();
        const commentId = (0, uuid_1.v4)();
        await db.run('INSERT INTO session_comments (id, session_id, user_id, comment) VALUES (?, ?, ?, ?)', [commentId, sessionId, userId, comment]);
    }
    /**
     * Get session comments
     */
    static async getSessionComments(sessionId, limit = 50, offset = 0) {
        const db = await (0, database_1.getDatabase)();
        return await db.all(`SELECT c.*, u.display_name, u.profile_image 
     ...
     * Search sessions
     */
    static async searchSessions(query, limit = 20, offset = 0) {
        const db = await (0, database_1.getDatabase)();
       ...
       LIMIT ? OFFSET ?`, [searchTerm, searchTerm, searchTerm, limit, offset]);
        return sessions.map((session) => ({
            ...session,
            chart_data: JSON.parse(session.chart_data),
            audio_config: session.audio_config ? JSON.parse(session.audio_config) : undefined,
