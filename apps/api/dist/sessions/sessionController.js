...
class SessionController {
    ...
    static async saveSession(req, res) {
        try {
            ...
            if (user.subscription_plan === 'free') {
                const sessionCount = await db.get('SELECT COUNT(*) as count FROM sessions WHERE user_id = ?', [req.user.id]);
    ...
    static async getUserSessions(req, res) {
      ...
      `, [req.user.id, Number(limit), Number(offset)]);
            const total = await db.get('SELECT COUNT(*) as count FROM sessions WHERE user_id = ?', [req.user.id]);
            res.json({
                success: true,
                data: {
    ...
    static async getSession(req, res) {
        try {
            ...
            if (req.user && session.user_id !== req.user.id) {
                await db.run('UPDATE sessions SET play_count = play_count + 1 WHERE id = ?', [id]);
            }
            res.json({
                success: true,
                data: {
    ...
    static async updateSession(req, res) {
        try {
            ...
            const updateFields = Object.keys(updates)
                .map(key => `${key} = ?`)
                .join(', ');
            await db.run(`UPDATE sessions SET ${updateFields} WHERE id = ?`, [...Object.values(updates), id]);
            res.json({
                success: true,
                data: {
                    id,
    ...
    static async getPublicSessions(req, res) {
      ...
      `, [Number(limit), Number(offset)]);
            const total = await db.get('SELECT COUNT(*) as count FROM sessions WHERE is_public = 1');
            res.json({
                success: true,
                data: {
                    sessions,
    ...
    static async toggleLike(req, res) {
        try {
            ...
            const existingLike = await db.get('SELECT * FROM session_likes WHERE session_id = ? AND user_id = ?', [id, req.user.id]);
            if (existingLike) {
                // Unlike
                await db.run('DELETE FROM session_likes WHERE session_id = ? AND user_id = ?', [id, req.user.id]);
                await db.run('UPDATE sessions SET like_count = like_count - 1 WHERE id = ?', [id]);
                res.json({
                    success: true,
                ...
                // Like
                await db.run('INSERT INTO session_likes (session_id, user_id, created_at) VALUES (?, ?, ?)', [id, req.user.id, new Date().toISOString()]);
                await db.run('UPDATE sessions SET like_count = like_count + 1 WHERE id = ?', [id]);
                res.json({
                    success: true,
