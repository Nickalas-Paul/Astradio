...
class FriendController {
    ...
    static async sendFriendRequest(req, res) {
        try {
            ...
            const existingRequest = await db.get('SELECT * FROM friends WHERE (user_id = ? AND friend_id = ?) OR (user_id = ? AND friend_id = ?)', [req.user.id, friend.id, friend.id, req.user.id]);
            if (existingRequest) {
            ...
            await db.run('INSERT INTO friends (id, user_id, friend_id, status, created_at) VALUES (?, ?, ?, ?, ?)', [friendRequestId, req.user.id, friend.id, 'pending', new Date().toISOString()]);
            res.status(201).json({
                success: true,
                data: {
                    id: friendRequestId,
    ...
    static async acceptFriendRequest(req, res) {
        try {
            ...
            const friendRequest = await db.get('SELECT * FROM friends WHERE id = ? AND friend_id = ? AND status = ?', [requestId, req.user.id, 'pending']);
            if (!friendRequest) {
                return res.status(404).json({
                    success: false,
            ...
            await db.run('UPDATE friends SET status = ? WHERE id = ?', ['accepted', requestId]);
            res.json({
                success: true,
                data: {
                    id: requestId,
    ...
    static async removeFriend(req, res) {
        try {
            ...
            const db = await (0, database_1.getDatabase)();
            // Find the friendship
            const friendship = await db.get('SELECT * FROM friends WHERE ((user_id = ? AND friend_id = ?) OR (user_id = ? AND friend_id = ?)) AND status = ?', [req.user.id, friendId, friendId, req.user.id, 'accepted']);
            if (!friendship) {
                return res.status(404).json({
                    success: false,
            ...
            await db.run('DELETE FROM friends WHERE ((user_id = ? AND friend_id = ?) OR (user_id = ? AND friend_id = ?))', [req.user.id, friendId, friendId, req.user.id]);
            res.json({
                success: true,
                data: {
     ...
     */
    static async getFriendSessions(req, res) {
        try {
            if (!req.user) {
                return res.status(401).json({
                    success: false,
    ...
    static async searchUsers(req, res) {
      ...
      `, [`%${query}%`, `%${query}%`, req.user.id]);
            ...
            const usersWithStatus = await Promise.all(users.map(async (user) => {
                const friendship = await db.get('SELECT status FROM friends WHERE ((user_id = ? AND friend_id = ?) OR (user_id = ? AND friend_id = ?))', [req.user.id, user.id, user.id, req.user.id]);
                return {
                    ...user,
