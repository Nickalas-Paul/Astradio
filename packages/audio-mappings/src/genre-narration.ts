...
import { 
  getGenreConfig, 
  getMoodMapping, 
  getTextStyleForGenre 
} from './genre-system';

export function generateGenreSpecificNarration(
  chartData: any, 
  genre: Genre, 
  mood?: Mood
): GenreSpecificNarration {
  const genreConfig = getGenreConfig(genre);
  const textStyle = getTextStyleForGenre(genre);
  const moodMapping = mood ? getMoodMapping(mood) : null;
...
function generateMusicalMood(chartData: any, genre: Genre, mood?: Mood): string {
  const textStyle = getTextStyleForGenre(genre);
  const moodConfig = mood ? getMoodMapping(mood) : null;
...
function generatePlanetaryExpression(chartData: any, genre: Genre, mood?: Mood): string {
  const textStyle = getTextStyleForGenre(genre);
...
function generateInterpretiveSummary(chartData: any, genre: Genre, mood?: Mood): string {
  const textStyle = getTextStyleForGenre(genre);
  const vocabulary = textStyle.vocabulary;
  const metaphors = textStyle.metaphors;
...
function getGenreSectionTitle(genre: Genre, section: string): string {
  const genreConfig = getGenreConfig(genre);
...
function combineNarrationSections(sections: NarrationSection[], genre: Genre): string {
  const genreConfig = getGenreConfig(genre);
  const textStyle = getTextStyleForGenre(genre);
...
function getGenreTransition(genre: Genre): string {
  switch (genre) {
    case 'classical':
...
export { generateGenreSpecificNarration as generateMusicNarration }; 