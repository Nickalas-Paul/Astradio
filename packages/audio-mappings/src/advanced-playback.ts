...
export interface PlaybackEffects {
  ...
  delay: number;       // 0-1: No delay to maximum
  distortion: number;  // 0-1: Clean to heavily distorted
  chorus: number;      // 0-1: No chorus to maximum
  filter: number;      // 0-1: Low pass filter intensity
}
...
export class AdvancedPlaybackEngine {
  private currentSession: MelodicAudioSession | null = null;
  private effects: PlaybackEffects;
   ...
   */
  addSegment(name: string, startTime: number, endTime: number, color: string = '#667eea', description?: string): void {
...
export const advancedPlaybackEngine = new AdvancedPlaybackEngine(); 