...
export function generateMusicNarration(
  ...
  const musicalConfig = getMusicalConfig(chartData, (config as any)?.genre || 'electronic');
  ...
  const elementMood = elementMoods[dominantElement as keyof typeof elementMoods];
  const modalityRhythm = modalityRhythms[dominantModality as keyof typeof modalityRhythms];
...
function generatePlanetaryExpression(
  chartData: AstroChart,
  aspects: any[],
  musicalConfig: any,
  config?: AudioConfiguration
  ...
  prominentPlanets.forEach(([planetName, planetData]) => {
    ...
    const genreVariation = mapping?.genreVariations?.[genre as keyof typeof mapping.genreVariations];
    ...
    if (planetAspects.length > 0) {
      ...
      const relationship = aspectRelationships[aspect.type as keyof typeof aspectRelationships];
  ...
  if (aspects.length > 0) {
    ...
    const relationship = aspectRelationships[mainAspect.type as keyof typeof aspectRelationships];
...
export function generateModeSpecificNarration(
  chartData: AstroChart,
  mode: string,
  config?: AudioConfiguration
): string {
...
export function generateDualChartNarration(
  chart1: AstroChart,
  chart2: AstroChart,
  config?: AudioConfiguration
): string {
