import { AstroChart, AudioSession, AudioConfiguration, GenreType } from '@astradio/types';
import { planetaryMappings } from './index';
...
export class EnhancedCompositionEngine {
  private currentSession: AudioSession | null = null;
  private scheduledEvents: any[] = [];

  // Duration configurations
  private readonly FREE_DURATION_PER_HOUSE = 5; // 5 seconds per house
  private readonly PREMIUM_DURATION_PER_HOUSE = 30; // 30 seconds per house
  private readonly FREE_TOTAL_DURATION = 60; // 1 minute total
  private readonly PREMIUM_TOTAL_DURATION = 360; // 6 minutes total
  ...
  async generateHouseBasedComposition(
    chartData: AstroChart, 
    config: CompositionConfig
  ): Promise<EnhancedComposition> {
    ...
    const houseCompositions: HouseComposition[] = [];
    const motifs: number[][] = [];
    let currentKey = 0; // C major

    for (let houseNum = 1; houseNum <= 12; houseNum++) {
      const planets = housesWithPlanets[houseNum] || [];
      const houseChar = this.houseCharacteristics[houseNum as keyof typeof this.houseCharacteristics];
      ...
      const houseComp: HouseComposition = {
  ...
  private organizePlanetsByHouse(chartData: AstroChart): Record<number, string[]> {
  ...
  private generateHouseMelody(
    ...
    const scale = this.elementScales[houseChar.element as keyof typeof this.elementScales];
    const notes: number[] = [];
    const rhythm: number[] = [];
  ...
  private generateHouseTransition(
    ...
    let transitionType: 'smooth' | 'contrast' | 'modulation' = 'smooth';
    let keyChange = 0;
  ...
  private calculateAverageTempo(houseCompositions: HouseComposition[]): number {
    const tempos = houseCompositions.map(comp => 
      this.houseCharacteristics[comp.houseNumber as keyof typeof this.houseCharacteristics].tempo
    );
    return Math.round(tempos.reduce((sum, tempo) => sum + tempo, 0) / tempos.length);
  }

  private generateHouseBasedInterpretation(
    houseCompositions: HouseComposition[], 
    config: CompositionConfig
  ): string {
    ...
    houseCompositions.forEach((house, index) => {
      if (house.planets.length === 0) {
        ...
        interpretation += `House ${house.houseNumber}: ${planetNames} ${house.planets.length === 1 ? 'influences' : 'influence'} the ${this.getHouseTheme(house.houseNumber)} area of your life. The ${this.houseCharacteristics[house.houseNumber as keyof typeof this.houseCharacteristics].mood} melody reflects this energy.\n\n`;
      }
    });
  ...
  async playComposition(composition: EnhancedComposition): Promise<AudioSession> {
...
export const enhancedCompositionEngine = new EnhancedCompositionEngine(); 