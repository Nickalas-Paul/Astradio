import { MelodicAudioSession, MelodicPhrase, MelodicNote, AudioSession, AstroChart } from '@astradio/types';
import { generateMusicNarration } from './narration-generator';
...
export class ExportEngine {
   ...
   */
  async exportToMIDI(session: MelodicAudioSession, options?: ExportOptions): Promise<ExportResult> {
    try {
      console.log(`🎵 Exporting session ${session.id} to MIDI format`);
      
      const midiData: MIDIExportData = {
        header: {
   ...
   */
  async exportToWAV(session: MelodicAudioSession, options?: ExportOptions): Promise<ExportResult> {
    try {
   ...
   */
  async exportToMP3(session: MelodicAudioSession, options?: ExportOptions): Promise<ExportResult> {
    try {
   ...
   */
  async exportNarration(session: MelodicAudioSession, chartData: any, options?: ExportOptions): Promise<ExportResult> {
    try {
      ...
      let exportData: NarrationExportData;
   ...
   */
  async batchExport(sessions: MelodicAudioSession[], chartData: any, options?: ExportOptions): Promise<ExportResult[]> {
  ...
  // Helper methods
  private convertToMIDITime(seconds: number, tempo: number): number {
  ...
  private calculateTempo(tempo: number): number {
    return Math.round(60000000 / tempo); // Microseconds per quarter note
  }

  private generateMarkdownNarration(narration: any, session: MelodicAudioSession, chartData: any): string {
...
`;
  ...
  private generateHTMLNarration(narration: any, session: MelodicAudioSession, chartData: any): string {
    return `<!DOCTYPE html>
<html>
<head>
...
<body>
  ...
  private generateMetadata(session: MelodicAudioSession, chartData: any): any {
    return {
