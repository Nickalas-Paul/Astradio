import { AstroChart, PlanetData } from '@astradio/types';

// Helper function to get sign number
function getSignNumber(signName: string): number {
...
export const enhancedPlanetaryMappings = {
  Sun: { 
    instrument: 'sawtooth', 
...
export function calculateAspects(chartData: AstroChart): any[] {
  const aspects = [];
    ...
    for (let j = i + 1; j < planets.length; j++) {
      ...
      if (conjunction) {
        aspects.push({
          ...
          harmonic: aspectMappings.conjunction.harmonic
        });
      } else if (sextile) {
        aspects.push({
          planet1, planet2, type: 'sextile', angle: angleDiff,
          harmonic: aspectMappings.sextile.harmonic
        });
      } else if (square) {
        aspects.push({
          planet1, planet2, type: 'square', angle: angleDiff,
          harmonic: aspectMappings.square.harmonic
        });
      } else if (trine) {
        aspects.push({
          planet1, planet2, type: 'trine', angle: angleDiff,
          harmonic: aspectMappings.trine.harmonic
        });
      } else if (opposition) {
        aspects.push({
          planet1, planet2, type: 'opposition', angle: angleDiff,
          harmonic: aspectMappings.opposition.harmonic
        });
      }
    }
  }
...
export function getMusicalConfig(chartData: AstroChart, genre: string = 'electronic'): any {
  ...
  planets.forEach(planet => {
    const element = enhancedPlanetaryMappings[planet as keyof typeof enhancedPlanetaryMappings]?.element;
    if (element) {
  ...
  return {
    genre,
    dominantElement,
    scale: elementScales[dominantElement as keyof typeof elementScales]?.[genre as keyof typeof elementScales.Fire] || elementScales.Fire[genre as keyof typeof elementScales.Fire],
    aspects,
    planets: planets.map(planet => ({
      name: planet,
      ...enhancedPlanetaryMappings[planet as keyof typeof enhancedPlanetaryMappings],
      genreConfig: enhancedPlanetaryMappings[planet as keyof typeof enhancedPlanetaryMappings]?.genreVariations?.[genre as keyof typeof enhancedPlanetaryMappings.Sun.genreVariations]
    })),
    instruments: genreInstruments[genre as keyof typeof genreInstruments]
  };
}

// Calculate harmonic intervals for aspects
export function calculateHarmonicInterval(baseFreq: number, aspectType: string): number {
  const aspect = aspectMappings[aspectType as keyof typeof aspectMappings];
  if (!aspect) return baseFreq;
...
export function getRhythmPattern(modality: string): any {
  const pattern = modalityMappings[modality as keyof typeof modalityMappings];
  if (!pattern) return { rhythm: 'quarter_notes', tempo: 'moderate' };
...
export function getTonalQuality(dignity: string): any {
  const quality = dignityMappings[dignity as keyof typeof dignityMappings];
  if (!quality) return { tonalQuality: 'neutral', volume: 1.0, clarity: 'clear' };
